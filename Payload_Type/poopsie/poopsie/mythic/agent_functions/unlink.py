...
class UnlinkArguments(TaskArguments):
    ...
    async def parse_arguments(self):
        if len(self.command_line) == 0:
            raise Exception("Unlink command requires arguments, but got empty command line.")
...
class UnlinkCommand(CommandBase):
    ...
    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        ...
        if 'callback_uuid' not in connection_info or connection_info["callback_uuid"] == "":
            ...
            response.Error = "Must select a callback to unlink from for webshell, not a payload"
            return response
        ...
        if len(callback_resp.Results) == 0:
            response.Success = False
            response.Error = "Failed to find callback to unlink from"
            return response
        return response
