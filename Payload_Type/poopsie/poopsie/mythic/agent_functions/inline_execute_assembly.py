    ...
    def __init__(self, command_line, **kwargs):
        ...
        self.args = [
            CommandParameter(
                ...
                description="Already existing .NET assembly to execute (e.g. Rubeus.exe)",
...
class InlineExecuteAssemblyCommand(CoffCommandBase):
    ...
    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        ...
        try:
            ...
            file_name = f"inline_execute_assembly.{taskData.Callback.Architecture}.o"
            arguments = [
                [
                    "bytes", 
                    assembly_contents.Content.hex()
                ],
                [
                    "int32",               
                    len(assembly_contents.Content)
                ],
                [
                    "wchar", 
                    taskData.args.get_arg("assembly_arguments")
            ...
            succeeded = await upload_if_missing(file_name=file_name, taskData=taskData, upload_type="BOF")
            if not succeeded:
