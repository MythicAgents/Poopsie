...
class ConnectArguments(TaskArguments):
    ...
    async def parse_arguments(self):
        if len(self.command_line) == 0:
            raise Exception("Connect command requires arguments, but got empty command line.")
...
class ConnectCommand(CommandBase):
    ...
    needs_admin = False
    help_cmd = "connect"
    description = "Connect to a new TCP agent on a remote host."
    version = 2
    author = "@djhohnstein"
    ...
    async def create_go_tasking(self, taskData: PTTaskMessageAllData) -> PTTaskCreateTaskingMessageResponse:
        ...
        if 'callback_uuid' not in connection_info or connection_info["callback_uuid"] == "":
            ...
            response.Error = "Must select a callback to connect to for webshell, not a payload"
            return response
        ...
        if len(callback_resp.Results) == 0:
            response.Success = False
            response.Error = "Failed to find callback to connect to"
            return response
        return response
